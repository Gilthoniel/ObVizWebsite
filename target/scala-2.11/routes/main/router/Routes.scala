...
class Routes(
  ...
  def documentation = List(
    ...
    ("""GET""", this.prefix + (if(this.prefix.endsWith("/")) "" else "/") + """details/$id<[^/]+>""", """controllers.Application.details(id:String)"""),
    Nil
  ...
  private[this] lazy val controllers_Application_details2_route = Route("GET",
    PathPattern(List(StaticPart(this.prefix), StaticPart(this.defaultPrefix), StaticPart("details/"), DynamicPart("id", """[^/]+""",true)))
  )
  private[this] lazy val controllers_Application_details2_invoker = createInvoker(
    Application_1.details(fakeValue[String]),
    HandlerDef(this.getClass.getClassLoader,
      "router",
      "controllers.Application",
      "details",
  ...
  def routes: PartialFunction[RequestHeader, Handler] = {
    ...
    case controllers_Application_details2_route(params) =>
      call(params.fromPath[String]("id", None)) { (id) =>
        controllers_Application_details2_invoker.call(Application_1.details(id))
      }
  }
}