...
public class Application extends Controller {
    ...
    public F.Promise<Result> index() {
        ...
        for (int i = 0; i < 20; i++) {
            apps.add(wb.getAppInfo(ids[i], Constants.Weight.LIGHT));
        }
    ...
    public F.Promise<Result> details(String id) {
        ...
        F.Promise<PlayApp> promise = wb.getAppInfo(id, Constants.Weight.FULL);
        F.Promise<Map<Integer, List<String>>> topics = wb.getTopicTitles();

        return promise.flatMap(playApp -> {

            List<F.Promise<PlayApp>> related = new ArrayList<>();
            for (String relatedID : playApp.getRelatedIDs()) {
                related.add(wb.getAppInfo(relatedID, Constants.Weight.LIGHT));
            }

            return F.Promise.sequence(related).flatMap(playApps -> {
                List<PlayApp> list = new ArrayList<>(playApps);
                Iterator<PlayApp> it = list.iterator();
                while (it.hasNext()) {
                    if (it.next() == null) {
                ...
                return topics.map(titles -> {
                    return ok(views.html.details.render(playApp, titles, list));
                });
            });
