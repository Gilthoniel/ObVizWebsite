...
public class Application extends Controller {
    ...
    public F.Promise<Result> index() {
        ...
        List<F.Promise<AndroidApp>> apps = new ArrayList<>();
        for (int i = 0; i < 20; i++) {
            apps.add(wb.getAppDetails(ids[i], Constants.Weight.LIGHT));
        }

        return F.Promise.sequence(apps).map(playApps -> {
            List<AndroidApp> list = new ArrayList<>(playApps);
            ...
            return ok(views.html.index.render(webpage, list));
        });
    }
        ...
        return promise.flatMap(app -> {
            ...
            return wb.getAppDetails(ids, Constants.Weight.LIGHT).flatMap(apps -> {
                return topics.map(titles -> {
                    ...
                    return ok(views.html.details.render(webpage, app, titles, apps));
                });
            });
        });
    }
